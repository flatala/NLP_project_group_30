...
class ContrieverRetriever(BaseRetriver):
    def __init__(
        self, config=DenseHyperParams, corpus_folder=None, corpus_file=None
    ) -> None:
        super().__init__()
        self.config = config
    ...
    def retrieve(
        ...
        **kwargs,
        ...
        if chunk:
            results = self.retrieve_in_chunks(
                corpus,
                queries,
                top_k=top_k,
        ...
        if index_present:
            ...
            print(f"Storing Embeddings at {path}")
        ...
        for idx, q in enumerate(queries):
            ...
            for index, id in enumerate(cos_scores_top_k_idx[idx]):
                response[q.id()][corpus[id].id()] = float(
                    cos_scores_top_k_values[idx][index]
                )
        return response
